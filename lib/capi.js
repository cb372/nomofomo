...
var getContentsWithTag = function(tag) {
  var fromDate = new Date();
  fromDate.setDate(fromDate.getDate() - 7);
  var fromDateParam = fromDate.getFullYear() + '-' + (fromDate.getMonth() + 1) + '-' + fromDate.getDate();
  return httpclient('http://content.guardianapis.com/search?api-key=gnm-hackday&show-fields=firstPublicationDate,shortUrl,thumbnail&page-size=20&order-by=relevance&from-date=' + fromDateParam + '&tag=' + tag)
  .then(function(capiResponse) {
